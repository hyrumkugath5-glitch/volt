# Building Argon as a downloadable app

Argon is an Electron app packaged with **electron-builder**. Icons live in
...
| File | What |
|------|------|
| `Argon-1.0.0-Setup.exe` | installer (Start-menu + desktop shortcut, choose install dir) |
| `Argon-1.0.0-portable.exe` | single file, double-click to run, nothing installed |
...
> window still show the Argon icon:
> ```bash
...
1. **On a Mac:** `npm ci && npm run dist:mac` → `dist/Argon-1.0.0-arm64.dmg` and
   `-x64.dmg` (Intel). Unsigned, so users right-click the app → **Open** the
   first time (or run `xattr -cr /Applications/Argon.app`).
...
```bash
npx electron-builder --linux         # -> dist/Argon-1.0.0-x86_64.AppImage
```

# Deploying Argon to the `games` Ubuntu server

Same shape as the chat app: build a full-snapshot tarball on Windows, extract it
over `~/argon-server` on the server, `npm install`, restart the service.

## First-time setup (on the server)

```bash
mkdir -p ~/argon-server
...
sudo cp deploy/argon.service /etc/systemd/system/argon.service
sudo systemctl daemon-reload
sudo systemctl enable --now argon
journalctl -u argon -f     # watch it start
```
...
you did for the chat http tunnel. Then Argon is at
...
front of Argon with a small reverse proxy, or run Argon behind the same nginx/caddy
you'd use for the chat app. (Argon itself is plain HTTP — put TLS in front.)
...
```bash
cd C:\Users\hyrum\argon\server
bash make-tarball.sh          # -> argon-server.tgz  (full snapshot, no diffs)
```

Copy `argon-server.tgz` to the server, then:

```bash
cd ~/argon-server
tar xzf ~/argon-server.tgz --strip-components=1
npm install --omit=dev
sudo systemctl restart argon
```
